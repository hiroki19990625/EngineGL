    ...
    public class CircleObject2D : DrawableObject
    {
        ...
        public override void OnDraw(double deltaTime)
        {
            ...
            for (float th1 = 0.0f; th1 <= 360.0f; th1 += 1.0f)
            {
                ...
                float x1 = Radius * (float) Math.Cos(th1_rad) * Bounds.X;
                float y1 = Radius * (float) Math.Sin(th1_rad) * Bounds.Y;
                float x2 = Radius * (float) Math.Cos(th2_rad) * Bounds.X;
                float y2 = Radius * (float) Math.Sin(th2_rad) * Bounds.Y;
